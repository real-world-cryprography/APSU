...
#include "apsu/bin_bundle_generated.h"
...
namespace apsu {
    ...
    namespace receiver {
        ...
        */
        ...
        {
            if (stripped_) {
                APSU_LOG_ERROR("Cannot insert data to a stripped BinBundle");
                throw logic_error("failed to insert data");
            }
            if (items.empty()) {
                APSU_LOG_ERROR("No item data to insert");
                return -1;
            }
            ...
            // size.
            if (get_label_size()) {
                APSU_LOG_ERROR("Attempted to insert unlabeled data in a labeled BinBundle");
                throw logic_error("failed to insert data");
            }
            ...
            bool dry_run)
        {
            ...
            if (item_labels.empty()) {
                APSU_LOG_ERROR("No item or label data to insert");
                return -1;
            }

            // We are inserting item-labels. This BinBundle cannot have a zero label size.
            if (!get_label_size()) {
                APSU_LOG_ERROR("Attempted to insert labeled data in an unlabeled BinBundle");
                throw logic_error("failed to insert data");
            }
        ...
        {
            if (stripped_) {
                APSU_LOG_ERROR("Cannot overwrite data in a stripped BinBundle");
        ...
        {
            if (stripped_) {
                APSU_LOG_ERROR("Cannot remove data from a stripped BinBundle");
                throw logic_error("failed to remove data");
            }
            if (items.empty()) {
                APSU_LOG_ERROR("No item data to remove");
                return false;
            }
        ...
        {
            if (stripped_) {
                APSU_LOG_ERROR("Cannot retrieve labels from a stripped BinBundle");
                throw logic_error("failed to retrieve labels");
            }
            if (items.empty()) {
                APSU_LOG_ERROR("No item data to search for");
                return false;
            }
        ...
        void BinBundle::regen_polyns()
        {
            ...
            // cout<<"one bundle time"<<std::chrono::duration<double> (dur).count() << std::endl;
        ...
        pair<uint32_t, size_t> BinBundle::load(gsl::span<const unsigned char> in)
        {
            ...
            if (!safe) {
                APSU_LOG_ERROR("Failed to load BinBundle: the buffer is invalid");
            ...
            if (!fits_in<flatbuffers::uoffset_t>(num_bins)) {
                APSU_LOG_ERROR("The loaded BinBundle has too many bins");
                throw runtime_error("failed to load BinBundle");
            }
            ...
            if (!fits_in<flatbuffers::uoffset_t>(label_size)) {
                APSU_LOG_ERROR("The loaded BinBundle has too large label size");
                throw runtime_error("failed to load BinBundle");
            }
            ...
            if (stripped_ && !bb->cache()) {
                APSU_LOG_ERROR("The loaded BinBundle is stripped but no cache data was found; this "
