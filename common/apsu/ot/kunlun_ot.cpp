...
namespace apsu{
  ...
  void ALSZ_KL_RandomSender(NetIO& chl,std::vector<std::array<osuCrypto::block, 2>>& sendMsg,size_t thread_num){
            ...
            ALSZOTE::RandomSend(chl,pp,vec_0,vec_1,exten_size);
            // Block::PrintBlocks(vec_0);
            // Block::PrintBlocks(vec_1);
            for(size_t idx = 0; idx < total_len; idx++){
                sendMsg[idx][0] = oc::toBlock((uint8_t*)&vec_0[idx]);
        ...
        void ALSZ_KL_RandomReceiver(NetIO& chl,osuCrypto::BitVector& choices,
            std::vector<osuCrypto::block>& recvMsg,
            size_t thread_num){
            CRYPTO_Initialize(); 
            ...
            ALSZOTE::RandomReceive(chl,pp,vec_k,bitChoice,exten_size);
            for(size_t idx = 0; idx < total_len; idx++){
                recvMsg[idx] = oc::toBlock((uint8_t*)&vec_k[idx]);
            }
                // for(auto x : bitChoice)
        ...
        void ALSZ_KL_ChosenSender(NetIO& chl,std::vector<std::array<oc::block, 2>> Messages){
            CRYPTO_Initialize(); 

            auto pp = ALSZOTE::Setup(128);
            ...
            ALSZOTE::OnesidedSend(chl,pp,vec_M,paddingOT);
            // Block::PrintBlocks(oneside_send);
        ...
        void ALSZ_KL_ChosenReceiver(NetIO& chl,std::vector<block>& DiffSet,size_t BlockNum,std::vector<size_t> choicesVec){
            CRYPTO_Initialize(); 
            ...
            DiffSet = ALSZOTE::OnesidedReceive(chl,pp,choices,PaddintLen);
