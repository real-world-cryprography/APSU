...
{ // Decode point P
  // SECURITY NOTE: this function does not run in constant time.
    ...
    if (sign != (unsigned int)sign_dec) { // If sign of x-coordinate decoded != input sign bit, then
                                          // negate x-coordinate
        fp2neg1271(P->x);
    }
...
{ // (Non-constant time) Montgomery inversion modulo the curve order using a^(-1) = a^(order-2) mod
  // order This function uses the sliding-window method
    sdigit_t i = 256;
    unsigned int j, nwords = NWORDS_ORDER;
    digit_t temp, bit = 0, count, mod2,
    ...
    // SECURITY NOTE: this function does not run in constant time because the modulus is assumed to
    // be public.
