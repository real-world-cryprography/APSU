...
{ // Generation of the precomputation table used by the variable-base scalar multiplication
  ...
  // extended twisted Edwards coordinates Output: table T containing 8 points: P, P+phi(P),
  // P+psi(P), P+phi(P)+psi(P), P+psi(phi(P)), P+phi(P)+psi(phi(P)), P+psi(P)+psi(phi(P)),
  // P+phi(P)+psi(P)+psi(phi(P)) Precomputed points use the representation (X+Y,Y-X,2Z,2dT)
  // corresponding to (X:Y:Z:T) in extended twisted Edwards coordinates
    ...
    eccadd_core(T[0], Q, PP); // T[1] = P+Q using the representations (X,Y,Z,Ta,Tb) <-
                              // (X+Y,Y-X,2Z,2dT) + (X+Y,Y-X,Z,T)
...
{ // Recoding sub-scalars for use in the variable-base scalar multiplication. See Algorithm 1 in
  // "Efficient and Secure Methods for GLV-Based Scalar Multiplication and their Implementation on
  // GLV-GLS Curves (Extended Version)", A. Faz-Hernandez, P. Longa, and A.H. Sanchez, in Journal of
  // Cryptographic Engineering, Vol. 5(1), 2015. Input: 4 64-bit sub-scalars passed through
  // "scalars", which are obtained after calling decompose(). Outputs: "digits" array with 65
  // nonzero entries. Each entry is in the range [0, 7], corresponding to one entry in the
  // precomputed table.
  //          "sign_masks" array with 65 entries storing the signs for their corresponding digits in
