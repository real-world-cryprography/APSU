...
// It produces a public key PublicKey, which is the encoding of P = s*G, where G is the generator
// and s is the output of hashing SecretKey and taking the least significant 32 bytes of the result.
...
// It produces a private key SecretKey and computes the public key PublicKey, which is the encoding
// of P = s*G, where G is the generator and s is the output of hashing SecretKey and taking the
// least significant 32 bytes of the result. Outputs: 32-byte SecretKey and 32-byte PublicKey
...
// SecretKey*G (G is the generator). Outputs: 32-byte SecretKey and 32-byte PublicKey
...
// The output is the y-coordinate of SecretKey*A, where A is the decoding of the public key
// PublicKey. Inputs: 32-byte SecretKey and 32-byte PublicKey Output: 32-byte SharedSecret
ECCRYPTO_STATUS CompressedSecretAgreement(
    const unsigned char *SecretKey, const unsigned char *PublicKey, unsigned char *SharedSecret);
...
// It produces a private key SecretKey and computes the public key PublicKey = SecretKey*G, where G
// is the generator. Outputs: 32-byte SecretKey and 64-byte PublicKey
...
ECCRYPTO_STATUS SecretAgreement(
    const unsigned char *SecretKey, const unsigned char *PublicKey, unsigned char *SharedSecret);
