...
{ // Constant-time table lookup to extract a point represented as (X+Y,Y-X,2Z,2dT) corresponding to
  // extended twisted Edwards coordinates (X:Y:Z:T) Inputs: sign_mask, digit, table containing 8
  // points Output: P = sign*table[digit], where sign=1 if sign_mask=0xFF...FF and sign=-1 if
  // sign_mask=0
    ...
    temp_point[0] = _mm256_loadu_si256(
        (__m256i *)point + 1); // point: x+y,y-x,2dt coordinate, temp_point: y-x,x+y,-2dt coordinate
    ...
    point[0] = _mm256_xor_si256(
        _mm256_and_si256(_mm256_xor_si256(point[0], temp_point[0]), full_mask), temp_point[0]);
    point[1] = _mm256_xor_si256(
        _mm256_and_si256(_mm256_xor_si256(point[1], temp_point[1]), full_mask), temp_point[1]);
    point[3] = _mm256_xor_si256(
        _mm256_and_si256(_mm256_xor_si256(point[3], temp_point[3]), full_mask), temp_point[3]);
    ...
    temp_point[0] = _mm256_loadu_pd(
        (double const *)point +
        4); // point: x+y,y-x,2dt coordinate, temp_point: y-x,x+y,-2dt coordinate
    ...
    point[0] = _mm256_blendv_pd(
        temp_point[0], point[0], full_mask); // If sign_mask = 0 then choose negative of the point
    ...
    fp2copy1271(
        point->xy,
        temp_point->yx); // point: x+y,y-x,2dt coordinate, temp_point: y-x,x+y,-2dt coordinate
    ...
    for (j = 0; j < NWORDS_FIELD; j++) { // If sign_mask = 0 then choose negative of the point
        point->xy[0][j] = ((digit_t)((int)sign_mask) & (point->xy[0][j] ^ temp_point->xy[0][j])) ^
                          temp_point->xy[0][j];
        point->xy[1][j] = ((digit_t)((int)sign_mask) & (point->xy[1][j] ^ temp_point->xy[1][j])) ^
                          temp_point->xy[1][j];
        point->yx[0][j] = ((digit_t)((int)sign_mask) & (point->yx[0][j] ^ temp_point->yx[0][j])) ^
                          temp_point->yx[0][j];
        point->yx[1][j] = ((digit_t)((int)sign_mask) & (point->yx[1][j] ^ temp_point->yx[1][j])) ^
                          temp_point->yx[1][j];
        point->t2[0][j] = ((digit_t)((int)sign_mask) & (point->t2[0][j] ^ temp_point->t2[0][j])) ^
                          temp_point->t2[0][j];
        point->t2[1][j] = ((digit_t)((int)sign_mask) & (point->t2[1][j] ^ temp_point->t2[1][j])) ^
                          temp_point->t2[1][j];
    }
...
{ // Constant-time table lookup to extract a point represented as (x+y,y-x,2t) corresponding to
  // extended twisted Edwards coordinates (X:Y:Z:T) with Z=1 Inputs: sign, digit, table containing
  // VPOINTS_FIXEDBASE = 2^(W_FIXEDBASE-1) points Output: if sign=0 then P = table[digit], else if
  // (sign=-1) then P = -table[digit]
    ...
    point[0] = _mm256_xor_si256(
        _mm256_and_si256(_mm256_xor_si256(point[0], temp_point[0]), full_mask), point[0]);
    point[1] = _mm256_xor_si256(
        _mm256_and_si256(_mm256_xor_si256(point[1], temp_point[1]), full_mask), point[1]);
    point[2] = _mm256_xor_si256(
        _mm256_and_si256(_mm256_xor_si256(point[2], temp_point[2]), full_mask), point[2]);
    _mm256_storeu_si256((__m256i *)P->xy, point[0]);
    _mm256_storeu_si256((__m256i *)P->yx, point[1]);
    ...
    temp_point[2] = _mm256_loadu_pd(
        (double const *)point +
        2 * 4); // point: x+y,y-x,2dt coordinate, temp_point: y-x,x+y,-2dt coordinate
    ...
    point[0] = _mm256_blendv_pd(
        point[0], temp_point[0], full_mask); // If sign = 0xFF...F then choose negative of the point
    point[1] = _mm256_blendv_pd(point[1], temp_point[1], full_mask);
    point[2] = _mm256_blendv_pd(point[2], temp_point[2], full_mask);
    _mm256_storeu_pd((double *)P->xy, point[0]);
    _mm256_storeu_pd((double *)P->yx, point[1]);
    ...
    for (j = 0; j < NWORDS_FIELD; j++) { // If sign = 0xFF...F then choose negative of the point
        point->xy[0][j] =
            ((digit_t)((int)sign) & (point->xy[0][j] ^ temp_point->xy[0][j])) ^ point->xy[0][j];
        point->xy[1][j] =
            ((digit_t)((int)sign) & (point->xy[1][j] ^ temp_point->xy[1][j])) ^ point->xy[1][j];
        point->yx[0][j] =
            ((digit_t)((int)sign) & (point->yx[0][j] ^ temp_point->yx[0][j])) ^ point->yx[0][j];
        point->yx[1][j] =
            ((digit_t)((int)sign) & (point->yx[1][j] ^ temp_point->yx[1][j])) ^ point->yx[1][j];
        point->t2[0][j] =
            ((digit_t)((int)sign) & (point->t2[0][j] ^ temp_point->t2[0][j])) ^ point->t2[0][j];
        point->t2[1][j] =
            ((digit_t)((int)sign) & (point->t2[1][j] ^ temp_point->t2[1][j])) ^ point->t2[1][j];
    }
