...
#include "apsu/fourq/FourQ_params.h"
#include "apsu/fourq/FourQ_tables.h"
#if defined(GENERIC_IMPLEMENTATION)
#include "apsu/fourq/generic/fp.h"
#elif (TARGET == TARGET_AMD64)
#include "apsu/fourq/amd64/fp_x64.h"
#elif (TARGET == TARGET_ARM64)
#include "apsu/fourq/arm64/fp_arm64.h"
#endif
...
{ // Clear integer-size digits from memory. "nwords" indicates the number of integer digits to be
  // zeroed. This function uses the volatile type qualifier to inform the compiler not to optimize
  // out the memory clearing. It has been tested with MSVS 2013 and GNU GCC 4.6.3, 4.7.3, 4.8.2
  // and 4.8.4. Users are responsible for verifying correctness with different compilers. See
  // "Compliant Solution (C99)" at
...
{ // Normalize a projective point (X1:Y1:Z1), including full reduction
  ...
  // Output: Q = (X1/Z1,Y1/Z1), corresponding to (X1:Y1:Z1:T1) in extended twisted Edwards
  // coordinates
...
{ // Conversion from representation (X,Y,Z,Ta,Tb) to (X+Y,Y-X,Z,T), where T = Ta*Tb
  // Input:  P = (X1,Y1,Z1,Ta,Tb), where T1 = Ta*Tb, corresponding to (X1:Y1:Z1:T1) in extended
  // twisted Edwards coordinates Output: Q = (X1+Y1,Y1-X1,Z1,T1) corresponding to (X1:Y1:Z1:T1) in
...
{ // Conversion from representation (X+Y,Y-X,2Z,2dT) to (2X,2Y,2Z,2dT)
  // Input:  P = (X1+Y1,Y1-X1,2Z1,2dT1) corresponding to (X1:Y1:Z1:T1) in extended twisted Edwards
  // coordinates Output: Q = (2X1,2Y1,2Z1) corresponding to (X1:Y1:Z1) in twisted Edwards
  // coordinates
...
{ // Basic point addition R = P+Q or R = P+P
  // Inputs: P = (X1+Y1,Y1-X1,2Z1,2dT1) corresponding to (X1:Y1:Z1:T1) in extended twisted Edwards
  // coordinates
  //         Q = (X2+Y2,Y2-X2,Z2,T2) corresponding to (X2:Y2:Z2:T2) in extended twisted Edwards
  //         coordinates
...
{ // Complete point addition P = P+Q or P = P+P
  // Inputs: P = (X1,Y1,Z1,Ta,Tb), where T1 = Ta*Tb, corresponding to (X1:Y1:Z1:T1) in extended
  // twisted Edwards coordinates
  //         Q = (X2+Y2,Y2-X2,2Z2,2dT2) corresponding to (X2:Y2:Z2:T2) in extended twisted Edwards
  //         coordinates
...
{ // Point conversion to representation (X,Y,Z,Ta,Tb)
  ...
  // Output: P = (X,Y,1,Ta,Tb), where Ta=X, Tb=Y and T=Ta*Tb, corresponding to (X:Y:Z:T) in extended
  // twisted Edwards coordinates
...
{ // Point validation: check if point lies on the curve
  ...
  // SECURITY NOTE: this function does not run in constant time (input point P is assumed to be
  // public).
    f2elm_t t1, t2, t3;
...
{ // Conversion from representation (x+y,y-x,2dt) to (X,Y,Z,Ta,Tb)
  // Input:  P = (x1+y1,y1-x1,2dt1) corresponding to (X1:Y1:Z1:T1) in extended twisted Edwards
  // coordinates, where Z1=1 Output: Q = (x1,y1,z1,x1,y1), where z1=1, corresponding to
  // (X1:Y1:Z1:T1) in extended twisted Edwards coordinates
...
{ // Mixed point addition P = P+Q or P = P+P
  // Inputs: P = (X1,Y1,Z1,Ta,Tb), where T1 = Ta*Tb, corresponding to (X1:Y1:Z1:T1) in extended
  // twisted Edwards coordinates
  //         Q = (x2+y2,y2-x2,2dt2) corresponding to (X2:Y2:Z2:T2) in extended twisted Edwards
  //         coordinates, where Z2=1
  // Output: P = (Xfinal,Yfinal,Zfinal,Tafinal,Tbfinal), where Tfinal = Tafinal*Tbfinal,
  //         corresponding to (Xfinal:Yfinal:Zfinal:Tfinal) in extended twisted Edwards coordinates
    f2elm_t t1, t2;
...
{ // Fixed-base scalar multiplication Q = k*G, where G is the generator. FIXED_BASE_TABLE stores
  // v*2^(w-1) = 80 multiples of G. Inputs: scalar "k" in [0, 2^256-1]. Output: Q = k*G in affine
  // coordinates (x,y). The function is based on the modified LSB-set comb method, which converts
  // the scalar to an odd signed representation with (bitlength(order)+w*v) digits.
    ...
    for (j = 0; j < (v - 1); j++) {
        ...
        table_lookup_fixed_base(
            ((point_precomp_t *)&FIXED_BASE_TABLE) + (v - j - 2) * (1 << (w - 1)),
            S,
            digit,
            digits[d - (j + 1) * e - 1]);
        eccmadd(
...
{ // Computes the modified LSB-set representation of a scalar
  ...
  // Output: digits, where the first "d" values (from index 0 to (d-1)) store the signs for the
  // recoded values using the convention: -1 (negative), 0 (positive), and
  //         the remaining values (from index d to (l-1)) store the recoded values in mLSB-set
  //         representation, excluding their sign, where l = d*w and d =
  //         ceil(bitlength(order)/(w*v))*v. The values v and w are fixed and must be in the range
  //         [1, 10] (see FourQ.h); they determine the size of the precomputed table
  //         "FIXED_BASE_TABLE" used by ecc_mul_fixed().
    ...
    for (i = 0; i < (d - 1); i++) {
        digits[i] = (unsigned int)((scalar[0] & 1) - 1); // Convention for the "sign" row:
                                                         // if scalar_(i+1) = 0 then digit_i = -1
                                                         // (negative), else if scalar_(i+1) = 1
                                                         // then digit_i = 0 (positive)
        // Shift scalar to the right by 1
        for (j = 0; j < (NWORDS64_ORDER - 1); j++) {
            SHIFTR(scalar[j + 1], scalar[j], 1, scalar[j], RADIX64);
        }
    ...
    for (i = d; i < l; i++) {
        digits[i] = (unsigned int)(scalar[0] & 1); // digits_i = k mod 2. Sign is determined by the
                                                   // "sign" row
        ...
        temp = (0 - digits[i - (i / d) * d]) &
               digits[i]; // if (digits_i=0 \/ 1) then temp = 0, else if (digits_i=-1) then temp = 1
...
{ // Double scalar multiplication R = k*G + l*Q, where the G is the generator. Uses
  // DOUBLE_SCALAR_TABLE, which contains multiples of G, Phi(G), Psi(G) and Phi(Psi(G)). Inputs:
  ...
  // The function uses wNAF with interleaving.
    ...
    // SECURITY NOTE: this function is intended for a non-constant-time operation such as signature
    // verification.
    ...
    point_extproj_precomp_t U, Q_table1[NPOINTS_DOUBLEMUL_WQ], Q_table2[NPOINTS_DOUBLEMUL_WQ],
        Q_table3[NPOINTS_DOUBLEMUL_WQ], Q_table4[NPOINTS_DOUBLEMUL_WQ];
    ...
    for (i = 64; i >= 0; i--) {
        ...
        if (digits_l1[i] < 0) {
            ...
            position = (digits_l1[i]) / 2; // Take U = (X_U,Y_U,Z_U,Td_U) <- (X+Y,Y-X,2Z,2dT) from a
                                           // point in the precomputed table
            eccadd(Q_table1[position], T); // T = T+U = (X_T,Y_T,Z_T,Ta_T,Tb_T) =
                                           // (X_T,Y_T,Z_T,Ta_T,Tb_T) + (X_U,Y_U,Z_U,Td_U)
        }
        if (digits_l2[i] < 0) {
        ...
        if (digits_k1[i] < 0) {
            ...
            position = (digits_k1[i]) / 2; // Take V = (X_V,Y_V,Z_V,Td_V) <- (x+y,y-x,2dt) from a
                                           // point in the precomputed table
            eccmadd(
                ((point_precomp_t *)&DOUBLE_SCALAR_TABLE)[position],
                T); // T = T+V = (X_T,Y_T,Z_T,Ta_T,Tb_T) = (X_T,Y_T,Z_T,Ta_T,Tb_T) +
                    // (X_V,Y_V,Z_V,Td_V)
        }
        if (digits_k2[i] < 0) {
...
{ // Generation of the precomputation table used internally by the double scalar multiplication
  // function ecc_mul_double(). Inputs: point P in representation (X,Y,Z,Ta,Tb),
  ...
  // Output: Table containing multiples of the base point P using representation (X+Y,Y-X,2Z,2dT).
    ...
    for (i = 1; i < npoints; i++) {
        eccadd_core(Table[i - 1], PP, Q); // Table[i] = Table[i-1]+2P using the representations
                                          // (X,Y,Z,Ta,Tb) <- (X+Y,Y-X,2Z,2dT) + (X+Y,Y-X,Z,T)
