...
using namespace std;
using namespace oc;
void OSNReceiver::rand_ot_send(std::vector<std::array<osuCrypto::block, 2>>& sendMsg, oc::Socket chl)
{
	//std::cout << "\n OT sender!! \n";
	...
	auto routine = [&](size_t tid)
	{
		...
		cp::sync_wait(baseOTs.receive(baseChoice, baseRecv, prng1,chl));
		std::cout << __FILE__ << __LINE__ << std::endl;

		sender.setBaseOts(baseRecv, baseChoice);

		cp::sync_wait(sender.send(tmpMsg, prng1, chl));
...
void OSNReceiver::silent_ot_send(std::vector<std::array<osuCrypto::block, 2>>& sendMsg,  oc::Socket chl)
{
	//std::cout << "\n Silent OT sender!! \n";
	...
	auto routine = [&](size_t tid)
	{
		...
		std::vector<std::array<osuCrypto::block, 2>> tmpMsg(size);
		sender.silentSend(tmpMsg, prng1, chl);
		std::copy_n(tmpMsg.begin(), size, sendMsg.begin() + start_idx);
	};
...
void OSNReceiver::Kunlun_ot_send(std::vector<std::array<oc::block, 2>>& sendMsg){
	

}

std::vector<std::vector<oc::block>> OSNReceiver::gen_benes_client_osn(int values, oc::Socket chl, NetIO& RecvChl)
{
	...
	oc::block temp;
	std::vector<oc::block> masks(values);
	std::vector<std::vector<oc::block>> ret_masks(values);

	osuCrypto::PRNG prng(_mm_set_epi32(4253233465, 334565, 0, 235));

	for (int j = 0; j < values; j++)
	{ // we sample the input masks randomly
		temp = prng.get<oc::block>();
		masks[j] = temp;
	...
	if (ot_type == OT_type::Silent_OT)
	{
		...
		silent_ot_send(tmp_messages, chl); // sample random ot blocks
	...
	if(ot_type == OT_type::RandOT)
	{
		std::vector<std::array<osuCrypto::block, 2>> tmp_messages(switches);
		rand_ot_send(tmp_messages, chl); // sample random ot blocks
		oc::AES aes(ZeroBlock);
		for (auto i = 0; i < ot_messages.size(); i++)
		{
			ot_messages[i][0] = { tmp_messages[i][0], aes.ecbEncBlock(tmp_messages[i][0]) };
	...
	RecvChl.SendBits((uint8_t*)correction_blocks.data(),switches*2*sizeof(oc::block));
	
	// for(auto ar : correction_blocks){
...
OSNReceiver::OSNReceiver(size_t size, OT_type ot_type) : size(size), ot_type(ot_type)
{
}

void OSNReceiver::init(size_t size, OT_type ot_type, size_t threads)
{
	this->size = size;
...
std::vector<oc::block> OSNReceiver::run_osn(std::vector<oc::block> inputs,  oc::Socket chl, NetIO& RecvChl)
{
	int values = size;
	std::cout << __LINE__ << std::endl;

	std::vector<std::vector<oc::block>> ret_masks = gen_benes_client_osn(values, chl,RecvChl);
	std::vector<oc::block> output_masks, benes_input;
	...
	// chl.send(benes_input);
	RecvChl.SendBits((uint8_t*)benes_input.data(),sizeof(oc::block)*values);
	// for(auto x : benes_input){
	...
	std::vector<std::array<osuCrypto::block, 2>>& correction_blocks)
{
	...
	int values = src.size();
	std::vector<oc::block> bottom1;
	std::vector<oc::block> top1;

	oc::block m0, m1, w0, w1, M0[2], M1[2], corr_mesg[2];
