...
namespace apsu {
    ...
    size_t PSUParams::save(ostream &out) const
    {
        ...
        fbs::PSUParamsBuilder psu_params_builder(fbs_builder);
        psu_params_builder.add_version(apsu_serialization_version);
        psu_params_builder.add_item_params(&item_params);
        psu_params_builder.add_table_params(&table_params);
        psu_params_builder.add_query_params(query_params);
        psu_params_builder.add_seal_params(seal_params);
        auto psu_params = psu_params_builder.Finish();
        fbs_builder.FinishSizePrefixed(psu_params);
    ...
    pair<PSUParams, size_t> PSUParams::Load(istream &in)
    {
        vector<unsigned char> in_data(util::read_from_stream(in));

        auto verifier = flatbuffers::Verifier(
            reinterpret_cast<const uint8_t *>(in_data.data()), in_data.size());
        bool safe = fbs::VerifySizePrefixedPSUParamsBuffer(verifier);
        if (!safe) {
            throw runtime_error("failed to load parameters: invalid buffer");
        }

        auto psu_params = fbs::GetSizePrefixedPSUParams(in_data.data());

        if (!same_serialization_version(psu_params->version())) {
            // Check that the serialization version numbers match
            APSU_LOG_ERROR(
                "Loaded PSUParams data indicates a serialization version number ("
                << psu_params->version()
        ...
        PSUParams::ItemParams item_params;
        item_params.felts_per_item = psu_params->item_params()->felts_per_item();

        PSUParams::TableParams table_params;
        table_params.table_size = psu_params->table_params()->table_size();
        table_params.max_items_per_bin = psu_params->table_params()->max_items_per_bin();
        table_params.hash_func_count = psu_params->table_params()->hash_func_count();

        PSUParams::QueryParams query_params;
        query_params.ps_low_degree = psu_params->query_params()->ps_low_degree();
        copy(
            psu_params->query_params()->query_powers()->cbegin(),
            psu_params->query_params()->query_powers()->cend(),
            inserter(query_params.query_powers, query_params.query_powers.end()));

        PSUParams::SEALParams seal_params;
        auto &seal_params_data = *psu_params->seal_params()->data();
        try {
            seal_params.load(
        ...
        return { PSUParams(item_params, table_params, query_params, seal_params), in_data.size() };
    }
#ifndef APSU_DISABLE_JSON
    PSUParams PSUParams::Load(const string &in)
    {
        // Parse JSON string
        Json::Value root;
        stringstream ss(in);
        ...
        try {
            ...
            APSU_LOG_ERROR("Failed to load table_params from JSON string: " << ex.what());
            throw;
        }
        ...
        try {
            ...
            APSU_LOG_ERROR("Failed to load item_params from JSON string: " << ex.what());
            throw;
        }
        ...
        try {
            ...
            APSU_LOG_ERROR("Failed to load query_params from JSON string: " << ex.what());
            throw;
        }
        ...
        try {
            ...
            APSU_LOG_ERROR("Failed to load seal_params from JSON string: " << ex.what());
            throw;
        }

        return PSUParams(item_params, table_params, query_params, seal_params);
    }
#else
    PSUParams PSUParams::Load(const string &in)
    {
...
#endif
    string PSUParams::to_string() const
    {
        stringstream ss;
