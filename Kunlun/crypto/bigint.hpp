...
inline BigInt::BigInt(const BIGNUM *other){
    this->bn_ptr = BN_new(); 
    BN_copy(this->bn_ptr, other); 
}

// Creates a new BigInt object from the number.
inline BigInt::BigInt(size_t number){
    this->bn_ptr = BN_new();
...
inline uint64_t BigInt::ToUint64() const
{
...
inline void BigInt::FromByteString(const std::string& str) 
{ 
...
inline void BigInt::FromByteString(const unsigned char* buffer, size_t LEN)
{ 
    BN_bin2bn(buffer, LEN, this->bn_ptr);
}

// an ad-hoc implementation for fixed length conversion: default LEN = BN_BYTE_LEN
inline void BigInt::ToByteString(unsigned char* buffer, size_t LEN) const
{
    BN_bn2bin(this->bn_ptr, buffer);
}  

inline std::string BigInt::ToByteString() const
{
...
inline std::string BigInt::ToHexString() const
{
    std::stringstream ss; 
...
inline void BigInt::FromByteVector(const std::vector<uint8_t>& vec_A)
{ 
    BN_bin2bn(reinterpret_cast<const unsigned char *>(vec_A.data()), vec_A.size(), this->bn_ptr);         
}

inline std::vector<uint8_t> BigInt::ToByteVector(size_t LEN) const
{ 
...
// Causes a check failure if the operation fails.
inline BigInt BigInt::Negate() const {
    BigInt result = *this;
...
// Causes a check failure if the operation fails.
inline BigInt BigInt::Add(const BigInt& other) const {
    BigInt result;
...
// Returns a BigInt whose value is (*this - val).
// Causes a check failure if the operation fails.
inline BigInt BigInt::Sub(const BigInt& other) const {
    BigInt result;
...
// Returns a BigInt whose value is (*this * val).
// Causes a check failure if the operation fails.
inline BigInt BigInt::Mul(const BigInt& other) const {
    BigInt result;
...
// Returns a BigInt whose value is (*this / val).
// Causes a check failure if the remainder != 0 or if the operation fails.
inline BigInt BigInt::Div(const BigInt& other) const {
    BigInt result;
    BigInt remainder;
    int thread_num = omp_get_thread_num();
...
// Returns a BigInt whose value is *this / val, rounding towards zero.
// Causes a check failure if the remainder != 0 or if the operation fails.
inline BigInt BigInt::DivAndTruncate(const BigInt& other) const {
    BigInt result;
    BigInt remainder;
    int thread_num = omp_get_thread_num();
    CRYPTO_CHECK(1 == BN_div(result.bn_ptr, remainder.bn_ptr, this->bn_ptr, other.bn_ptr, bn_ctx[thread_num]));
...
inline int BigInt::CompareTo(const BigInt& other) const {
    return BN_cmp(this->bn_ptr, other.bn_ptr);
}

// Returns a BigInt whose value is (*this ^ exponent).
// Causes a check failure if the operation fails.
inline BigInt BigInt::Exp(const BigInt& exponent) const{
    BigInt result;
    int thread_num = omp_get_thread_num();
...
// return square
inline BigInt BigInt::Square() const{
    BigInt result;
    int thread_num = omp_get_thread_num();
...
// Returns a BigInt whose value is (*this mod m).
inline BigInt BigInt::Mod(const BigInt& modulus) const {
    BigInt result;
    int thread_num = omp_get_thread_num();
...
// Causes a check failure if the operation fails.
inline BigInt BigInt::ModAdd(const BigInt& other, const BigInt& modulus) const {
    BigInt result;
    int thread_num = omp_get_thread_num();
...
// Returns a BigInt whose value is (*this - val mod m).
// Causes a check failure if the operation fails.
inline BigInt BigInt::ModSub(const BigInt& other, const BigInt& modulus) const {
    BigInt result;
    int thread_num = omp_get_thread_num();
...
// Returns a BigInt whose value is (*this * val mod m).
inline BigInt BigInt::ModMul(const BigInt& other, const BigInt& modulus) const {
    BigInt result;
    int thread_num = omp_get_thread_num();
...
// Returns a BigInt whose value is (*this ^ exponent mod m).
// Causes a check failure if the operation fails.
inline BigInt BigInt::ModExp(const BigInt& exponent, const BigInt& modulus) const {
...
// Return a BigInt whose value is (*this^2 mod m).
// Causes a check failure if the operation fails.
inline BigInt BigInt::ModSquare(const BigInt& modulus) const {
    BigInt result;
    int thread_num = omp_get_thread_num();
...
// Returns a BigInt whose value is (*this ^ -1 mod m).
// Causes a check failure if the operation fails.
inline BigInt BigInt::ModInverse(const BigInt& modulus) const {
    BigInt result;
    int thread_num = omp_get_thread_num();
...
// Returns r such that r^2 == *this mod p.
// Causes a check failure if the operation fails.
inline BigInt BigInt::ModSquareRoot(const BigInt& modulus) const {
    BigInt result;
    int thread_num = omp_get_thread_num();
...
// Computes -a mod m.
// Causes a check failure if the operation fails.
inline BigInt BigInt::ModNegate(const BigInt& modulus) const {
    if (IsZero()) {
        return *this;
    }
...
// Returns a BigInt whose value is (*this >> n).
inline BigInt BigInt::Lshift(int n) const {
    BigInt result;
...
// Returns a BigInt whose value is (*this << n).
// Causes a check failure if the operation fails.
inline BigInt BigInt::Rshift(int n) const {
    BigInt result;
...
// Causes a check failure if the operation fails.
inline BigInt BigInt::GCD(const BigInt& other) const {
    BigInt result;
    int thread_num = omp_get_thread_num();
...
inline bool BigInt::IsPrime(double prime_error_probability) const {
...
inline bool BigInt::IsSafePrime(double prime_error_probability = 1e-40) const {
...
inline BigInt GenRandomBigIntLessThan(const BIGNUM* max) {
    BigInt result;
...
inline BigInt GenRandomBigIntLessThan(const BigInt& max) {
    BigInt result;
...
inline BigInt GenRandomBigIntBetween(const BigInt& start, const BigInt& end) {
    if (start > end) {
...
inline std::string GenRandomBytes(int num_bytes) {
    if (num_bytes < 0){
...
inline BigInt GenCoPrimeLessThan(const BigInt& num) {
...
inline BigInt GenSafePrime(int prime_length) {
    BigInt result;
...
inline BigInt GenPrime(int prime_length) {
    BigInt result;
...
inline void BigInt::Print(std::string note) const
{
    std::cout << note << " = "; 
    this->Print();
}

inline void BigInt::PrintInDec(std::string note) const
{
    std::cout << note << " = "; 
    char *bn_str;
...
inline void BigInt::PrintInDec() const
{ 
    char *bn_str;
    bn_str = BN_bn2dec(this->bn_ptr);  
...
inline int BigInt::GetTheNthBit(size_t j) const
{
    BigInt a = *this;  
    a = a >> j;
    a = a.GetLastNBits(1);  
...
inline std::ofstream &operator<<(std::ofstream &fout, const BigInt& a)
{ 
...
inline std::ifstream &operator>>(std::ifstream &fin, BigInt &a)
{ 
...
inline std::ofstream &operator<<(std::ofstream &fout, const std::vector<BigInt>& vec_a)
{ 
...
inline std::ifstream &operator>>(std::ifstream &fin, std::vector<BigInt>& vec_a)
{ 
    for(auto i = 0; i < vec_a.size(); i++){
        fin >> vec_a[i]; 
    }
...
inline void PrintBigIntVector(std::vector<BigInt> &vec_a, std::string note)
{
    for (auto i = 0; i < vec_a.size(); i++)
    {
        std::cout << note <<"[" << i << "]="; 
...
inline std::vector<BigInt> BigIntVectorModAdd(std::vector<BigInt> &vec_a, std::vector<BigInt> &vec_b, const BigInt &modulus)
{
...
/* a[i] = (a[i]-b[i]) mod modulus */ 
inline std::vector<BigInt> BigIntVectorModSub(std::vector<BigInt> &vec_a, std::vector<BigInt> &vec_b, const BigInt& modulus)
{
    if (vec_a.size() != vec_b.size()) {
...
inline std::vector<BigInt> BigIntVectorModProduct(std::vector<BigInt> &vec_a, std::vector<BigInt> &vec_b, const BigInt& modulus)
{
    if (vec_a.size() != vec_b.size()) {
        std::cerr << "vector size does not match!" << std::endl;
        exit(EXIT_FAILURE); 
...
/* c[i] = a[i]*b[i] */ 
inline std::vector<BigInt> BigIntVectorProduct(std::vector<BigInt> &vec_a, std::vector<BigInt> &vec_b)
{
    if (vec_a.size() != vec_b.size()) {
        std::cerr << "vector size does not match!" << std::endl;
        exit(EXIT_FAILURE); 
...
inline std::vector<BigInt> BigIntVectorModInverse(std::vector<BigInt> &vec_a, const BigInt& modulus)
{
    size_t LEN = vec_a.size(); 
    std::vector<BigInt> vec_result(LEN);
...
/* result[i] = c * a[i] */  
inline std::vector<BigInt> BigIntVectorModScalar(std::vector<BigInt> &vec_a, BigInt &c, const BigInt& modulus)
{
    size_t LEN = vec_a.size();
    std::vector<BigInt> vec_result(LEN);
...
/* result[i] = c * a[i] */  
inline std::vector<BigInt> BigIntVectorScalar(std::vector<BigInt> &vec_a, BigInt &c)
{
    size_t LEN = vec_a.size();
    std::vector<BigInt> vec_result(LEN); 
...
/* result[i] = -result[i] */  
inline std::vector<BigInt> BigIntVectorModNegate(std::vector<BigInt> &vec_a, BigInt &modulus)
{
    size_t LEN = vec_a.size();
    std::vector<BigInt> vec_result(LEN); 
...
/* sum_i^n a[i]*b[i] */
inline BigInt BigIntVectorModInnerProduct(std::vector<BigInt> &vec_a, std::vector<BigInt> &vec_b, const BigInt& modulus)
{
    BigInt result = bn_0; 
...
/* sum_i^n a[i]*b[i] */
inline BigInt BigIntVectorInnerProduct(std::vector<BigInt> &vec_a, std::vector<BigInt> &vec_b, const BigInt& modulus)
{
    BigInt result = bn_0; 
...
inline std::vector<BigInt> GenRandomBigIntVectorLessThan(size_t LEN, const BigInt &modulus)
{
    std::vector<BigInt> vec_result(LEN);
