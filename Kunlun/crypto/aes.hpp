...
// print pp
inline void PrintKey(const Key &key)
{
...
inline std::ofstream &operator<<(std::ofstream &fout, const Key &key)
{
    for(auto i = 0; i < 11; i++){
        fout << key.roundkey[i]; 
    }
...
inline std::ifstream &operator>>(std::ifstream &fin, Key &key)
{
    for(auto i = 0; i < 11; i++){
        fin >> key.roundkey[i]; 
    }
