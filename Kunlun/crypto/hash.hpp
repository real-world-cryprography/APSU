...
inline void SM3(const unsigned char *input, size_t HASH_INPUT_LEN, unsigned char *output)
{
    EVP_MD_CTX *md_ctx;
    const EVP_MD *md;
...
inline void CBCAES(const unsigned char *input, size_t HASH_INPUT_LEN, unsigned char *output) 
{
...
inline block StringToBlock(const std::string &str_input) 
{
...
__attribute__((target("sse2")))
inline block BytesToBlock(const std::vector<uint8_t> &vec_A) 
{
    unsigned char output[HASH_OUTPUT_LEN];
    //const unsigned char* input = reinterpret_cast<const unsigned char*>(str_input.c_str());
    //size_t HASH_INPUT_LEN = str_input.length();
...
inline BigInt StringToBigInt(const std::string& str_input){
    unsigned char output[HASH_OUTPUT_LEN]; 
    const unsigned char* input = reinterpret_cast<const unsigned char*>(str_input.c_str());
    size_t HASH_INPUT_LEN = str_input.length();
...
inline ECPoint StringToECPoint(const std::string& input) 
{
    ECPoint ecp_result; 
...
inline block ECPointToBlock(const ECPoint &A) 
{
    std::string str_input = A.ToByteString();
    return StringToBlock(str_input);  
}

inline std::string ECPointToString(const ECPoint &A) 
{ 
...
inline block BlocksToBlock(const std::vector<block> &input_block)
{
...
inline block FastBlocksToBlock(const std::vector<block> input_block)
{
...
*/
inline int BlockToBytes(const block &var, uint8_t* output, size_t LEN)
{
