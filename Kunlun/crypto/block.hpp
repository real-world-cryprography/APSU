...
inline block GenMaskBlock(size_t n)
{
...
// set the n-th bit = 1
inline void SetBit(block &a, size_t n)
{
    a = _mm_or_si128(a, GenMaskBlock(n));
}

// set the n-th bit = 0
inline void ClearBit(block &a, size_t n)
{
...
inline void PrintBlocks(block* var, size_t LEN) 
{
    for(auto i = 0; i< LEN; i++){
        PrintBlock(var[i]); 
        //std::cout << std::endl; 
...
inline void PrintBlocks(std::vector<block> vec_B) 
{
...
inline std::ofstream &operator<<(std::ofstream &fout, const block &a)
{ 
    char buffer[16];
...
inline std::ifstream &operator>>(std::ifstream &fin, block &a)
{ 
    char buffer[16];
    fin.read(buffer, 16); 
...
inline std::ofstream &operator<<(std::ofstream &fout, const std::vector<block> &vec_a)
{ 
...
inline std::ifstream &operator>>(std::ifstream &fin, std::vector<block> &vec_a)
{ 
    size_t LEN = vec_a.size() * 16; 
    char *buffer = new char[LEN]();
    fin.read(buffer, LEN);
...
inline std::set<block, BlockCompare> ComputeSetDifference(std::vector<block> &vec_A, std::vector<block> &vec_B)
{ 
