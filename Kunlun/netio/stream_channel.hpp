...
inline NetIO::NetIO(std::string party, std::string address, int port)
{
...
inline void NetIO::SendDataInternal(const void *data, size_t LEN)
{
...
inline void NetIO::ReceiveDataInternal(const void *data, size_t LEN)
{
...
inline void NetIO::SendBytes(const void* data, size_t LEN) 
{
	SendDataInternal(data, LEN); 
}

inline void NetIO::ReceiveBytes(void* data, size_t LEN) 
{
	ReceiveDataInternal(data, LEN); 
}

inline void NetIO::SendBlocks(const block* data, size_t LEN) 
{
	SendBytes(data, LEN*sizeof(block));
}

inline void NetIO::ReceiveBlocks(block* data, size_t LEN) 
{
	ReceiveBytes(data, LEN*sizeof(block));
}

inline void NetIO::SendBits(uint8_t *data, size_t LEN) 
{
	SendBytes(data, LEN);
}

inline void NetIO::ReceiveBits(uint8_t *data, size_t LEN) 
{
	ReceiveBytes(data, LEN);
}

inline void NetIO::SendString(char *data, size_t LEN) 
{
	SendBytes(data, LEN);
}

inline void NetIO::ReceiveString(char *data, size_t LEN) 
{
	ReceiveBytes(data, LEN);  
}

inline void NetIO::SendString(std::string &str) 
{
	SendBytes(&str[0], str.size());
}

inline void NetIO::ReceiveString(std::string &str) 
{
	ReceiveBytes(&str[0], str.size()); 
}

inline void NetIO::SendECPoints(const ECPoint* A, size_t LEN) 
{
...
inline void NetIO::ReceiveECPoints(ECPoint* A, size_t LEN) 
{
	int thread_num = omp_get_thread_num();
	#ifdef ECPOINT_COMPRESSED
		unsigned char* buffer = new unsigned char[LEN*POINT_COMPRESSED_BYTE_LEN];
...
inline void NetIO::SendEC25519Points(const EC25519Point* A, size_t LEN) 
{
...
inline void NetIO::ReceiveEC25519Points(EC25519Point* A, size_t LEN) 
{
	unsigned char* buffer = new unsigned char[32*LEN];
...
inline void NetIO::SendECPoint(const ECPoint &A) 
{
	int thread_num = omp_get_thread_num();
	#ifdef ECPOINT_COMPRESSED
...
inline void NetIO::ReceiveECPoint(ECPoint &A) 
{
	int thread_num = omp_get_thread_num();
	#ifdef ECPOINT_COMPRESSED
		unsigned char buffer[POINT_COMPRESSED_BYTE_LEN];
...
inline void NetIO::SendBigInt(const BigInt &a) 
{
...
inline void NetIO::ReceiveBigInt(BigInt &a) 
{
	unsigned char buffer[BN_BYTE_LEN];
...
inline void NetIO::SendBigInt(const BigInt &a, size_t LEN)
{
...
inline void NetIO::ReceiveBigInt(BigInt &a, size_t LEN) 
{
	unsigned char buffer[LEN];
...
template <typename T>
inline void NetIO::SendInteger(const T &n)
{
	SendBytes(&n, sizeof(T));
}
template <typename T>
inline void NetIO::ReceiveInteger(T &n)
{
	ReceiveBytes(&n, sizeof(T));
}


inline void NetIO::SendBlock(const block &a) 
{
	SendBytes(&a, sizeof(block));
}

inline void NetIO::ReceiveBlock(block &a) 
{
...
inline void NetIO::SendBytesArray(const std::vector<std::vector<uint8_t>>& A) 
{
	size_t NUM = A.size(); 
	size_t LEN = A[0].size(); 
	SendInteger(NUM);
...
inline void NetIO::ReceiveBytesArray(std::vector<std::vector<uint8_t>> &A) 
{
	size_t NUM, LEN; 
	ReceiveInteger(NUM);
	ReceiveInteger(LEN);  
...
// NUM = length of array; LEN = length of each item
inline void NetIO::SendStringArray(const std::vector<std::string>& A, size_t LEN) 
{
	size_t NUM = A.size(); 
	SendInteger(NUM);
...
inline void NetIO::ReceiveStringArray(std::vector<std::string> &A, size_t LEN) 
{
	size_t NUM; 
	ReceiveInteger(NUM);  
