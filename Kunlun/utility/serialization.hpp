...
#ifdef IS_LINUX
inline std::ofstream &operator<<(std::ofstream &fout, const size_t& element)
{
    fout.write(reinterpret_cast<const char *>(&element), 8); 
    return fout; 
}

inline std::ifstream &operator>>(std::ifstream &fin, size_t& element)
{
...
inline std::ofstream &operator<<(std::ofstream &fout, const ElementType& element)
{
...
template <typename ElementType> // Note: T must be a C++ POD type.
inline std::ifstream &operator>>(std::ifstream &fin, ElementType& element)
{
...
template <typename ElementType> // Note: T must be a C++ POD type.
inline std::ofstream &operator<<(std::ofstream &fout, const std::vector<ElementType>& vec_element)
{
...
template <typename ElementType> // Note: T must be a C++ POD type.
inline std::ifstream &operator>>(std::ifstream &fin, std::vector<ElementType>& vec_element)
{ 
...
inline std::ofstream &operator<<<std::string>(std::ofstream &fout, const std::string& str)
{
    size_t LEN = str.size();
    fout << LEN; 
...
template <> // specialize for string
inline std::ifstream &operator>><std::string>(std::ifstream &fin, std::string& str)
{
    size_t LEN; 
    fin >> LEN;
    str.resize(LEN); 
