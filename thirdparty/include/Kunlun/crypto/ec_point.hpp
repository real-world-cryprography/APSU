...
inline ECPoint::ECPoint(){
    this->point_ptr = EC_POINT_new(group);
}

inline ECPoint::ECPoint(const ECPoint& other){
    this->point_ptr = EC_POINT_new(group);
    EC_POINT_copy(this->point_ptr, other.point_ptr);
}

inline ECPoint::ECPoint(const EC_POINT* &other){
	std::cout << __FILE__ << __LINE__ << std::endl;
    this->point_ptr = EC_POINT_new(group);
	std::cout << __FILE__ << __LINE__ << std::endl;
    std::cout << other << std::endl;
    std::cout << this->point_ptr << std::endl;

    auto res = EC_POINT_copy(this->point_ptr, other);
	std::cout << __FILE__ << __LINE__ << std::endl;
	std::cout << res << std::endl;

}

inline ECPoint::ECPoint(const BigInt& x, const BigInt& y){
    this->point_ptr = EC_POINT_new(group);
    EC_POINT_set_affine_coordinates_GFp(group, this->point_ptr, x.bn_ptr, y.bn_ptr, bn_ctx);
}

inline void ECPoint::ReInitialize(){
...
inline ECPoint ECPoint::Mul(const BigInt& scalar) const {
    ECPoint result; 
...
inline ECPoint ECPoint::Add(const ECPoint& other) const {  
...
inline ECPoint ECPoint::Invert() const {
...
inline ECPoint ECPoint::Sub(const ECPoint& other) const { 
...
inline bool ECPoint::IsValid() const{
...
inline void ECPoint::Print(std::string note) const
{ 
    std::cout << note << " = "; 
    this->Print(); 
}
...
inline std::string ECPoint::ToByteString() const
{
...
inline std::string ECPoint::ToHexString() const
{
    std::stringstream ss; 
...
inline size_t ECPoint::ToUint64() const
{
    // standard method
...
inline size_t ECPoint::FastToUint64() const 
{
...
inline std::ofstream &operator<<(std::ofstream &fout, const ECPoint &A)
{ 
    unsigned char buffer[POINT_COMPRESSED_BYTE_LEN];
...
inline std::ifstream &operator>>(std::ifstream &fin, ECPoint &A)
{ 
    unsigned char buffer[POINT_COMPRESSED_BYTE_LEN];
...
inline std::ofstream &operator<<(std::ofstream &fout, const std::vector<ECPoint> &vec_A)
{ 
...
inline std::ifstream &operator>>(std::ifstream &fin, std::vector<ECPoint> &vec_A)
{ 
    for(auto i = 0; i < vec_A.size(); i++){
        fin >> vec_A[i];
    }
...
inline ECPoint CreateECPoint(const BigInt& x, const BigInt& y)
{
...
inline ECPoint GenRandomGenerator()
{
...
inline ECPoint GetPointAtInfinity(){
    ECPoint result;
...
inline bool IsSquare(const BigInt& q) {
...
// mul exp operations
inline ECPoint ECPointVectorMul(const std::vector<ECPoint> &vec_A, std::vector<BigInt> &vec_a){
...
// mul exp operations
inline ECPoint ECPointVectorMul(const std::vector<ECPoint> &vec_A, std::vector<BigInt> &vec_a, size_t start_index, size_t end_index){
...
/* g[i] = g[i]+h[i] */ 
inline std::vector<ECPoint> ECPointVectorAdd(std::vector<ECPoint> &vec_A, std::vector<ECPoint> &vec_B)
{
...
inline std::vector<ECPoint> GenRandomECPointVector(size_t LEN)
{
    std::vector<ECPoint> vec_result(LEN); 
    #pragma omp parallel for num_threads(thread_count)
    for(auto i = 0; i < LEN; i++){ 
...
inline ECPoint GenRandomECPoint()
{
    return GenRandomGenerator(); 
}

// print an EC Point vector
inline void PrintECPointVector(const std::vector<ECPoint> &vec_A, std::string note)
{ 
...
inline auto ECPoint_Lexical_Compare = [](ECPoint A, ECPoint B){ 
