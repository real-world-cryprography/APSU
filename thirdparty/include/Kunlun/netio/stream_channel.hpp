...
inline void NetIO::SendECPoints(const ECPoint* A, size_t LEN) 
{
		...
		for(auto i = 0; i < LEN; i++) {
    		...
    		EC_POINT_point2oct(group, A[i].point_ptr, POINT_CONVERSION_UNCOMPRESSED, 
				               buffer + i*POINT_BYTE_LEN, POINT_BYTE_LEN, NULL);
    	}
		std::cout << LEN*POINT_BYTE_LEN << std::endl;
...
inline void NetIO::ReceiveECPoints(ECPoint* A, size_t LEN) 
{
		...
		std::cout << __FILE__ << __LINE__ << std::endl;
		std::cout << POINT_BYTE_LEN << std::endl;
		std::cout << LEN << std::endl;
		auto temp = LEN*POINT_BYTE_LEN;
		std::cout << temp << std::endl;

		for(size_t idx = 0; idx < LEN*POINT_BYTE_LEN ; idx++){
			printf("%02x",buffer[idx]);
		}
		std::cout << std::endl;

		EC_GROUP* st = EC_GROUP_new_by_curve_name(415);
		EC_POINT* p = EC_POINT_new(st);
		BN_CTX* ctx = BN_CTX_new();
		// EC_POINT_oct2point(group,A[0].point_ptr,buffer,POINT_BYTE_LEN,bn_ctx);
		std::cout << __FILE__ << __LINE__ << std::endl;

		for(auto i = 0; i < LEN; i++) {
			EC_POINT_oct2point(group, A[i].point_ptr, buffer+i*POINT_BYTE_LEN, POINT_BYTE_LEN, NULL);
			A[i].Print();
		}
...
inline void NetIO::SendECPoint(const ECPoint &A) 
{
		...
		EC_POINT_point2oct(group, A.point_ptr, POINT_CONVERSION_UNCOMPRESSED, buffer, POINT_BYTE_LEN, NULL);
...
inline void NetIO::ReceiveECPoint(ECPoint &A) 
{
		...
		EC_POINT_oct2point(group, A.point_ptr, buffer, POINT_BYTE_LEN, NULL);
	#endif
}
