...
inline void Send(NetIO &io, PP &pp, const std::vector<block>& vec_m0, const std::vector<block> &vec_m1, size_t LEN)
{	
	PrintSplitLine('-');
	...
	io.SendECPoints(vec_X.data(), LEN); 
        std::cout << __FILE__ << __LINE__ << std::endl;
	PrintECPointVector(vec_X,"vec_X");
	...
	PrintECPointVector(vec_X,"vec_pk0");
...
inline std::vector<block> Receive(NetIO &io, PP &pp, const std::vector<uint8_t> &vec_selection_bit, size_t LEN)
{	
	PrintSplitLine('-');
	std::cout << __FILE__ << __LINE__ << std::endl;
	...
	PrintECPointVector(vec_pk0,"vec_pk0");
