...
inline std::vector<BigInt> PolyMul(std::vector<BigInt> A, std::vector<BigInt> B)
{
...
// result = P[0]*...P[n-1]
inline std::vector<BigInt> PolyMul(std::vector<std::vector<BigInt>> P)
{
...
inline void PrintPoly(std::vector<BigInt> P)
{
    for (auto i = 0; i < P.size(); i++){
       P[i].PrintInDec();
       if (i != 0) std::cout << "*x^" << i;
...
// A utility function to print a polynomial
inline BigInt EvalPoly(std::vector<BigInt> P, BigInt x)
{
    BigInt result = bn_0;
    BigInt carry = bn_1;  
    for (auto i = 0; i < P.size(); i++){
